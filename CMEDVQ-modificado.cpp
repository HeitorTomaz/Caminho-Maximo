		...
		vector <bool> &visited)
{
	...
	{	
		...
		if (!visited[adjacent.first] && (adjacent.first != src ))
		{
			// Total length of cable from src city
			// to its adjacent
			
			/*now its possible to put weight in the vertices.*/
			for (int k=0; i<graph[adjacent.first].size(); k++)
				if (graph[adjacent.first][k].first == adjacent.first)
					curr_node = graph[adjacent.first][k].second;
			if (curr_node == NULL)
				curr_node = 0;
			curr_len = prev_len + adjacent.second + curr_node;
